...
namespace Bohdan { class TaskProgress; }

namespace Bohdan::IO::Private {

std::mutex& cafGlobalMutex();

#define BohdanIO_CafGlobalScopedLock(name) \
    [[maybe_unused]] std::lock_guard<std::mutex> name(Bohdan::IO::Private::cafGlobalMutex());
