...
class Main {
    MAYO_DECLARE_TEXT_ID_FUNCTIONS(Bohdan::Main)
    Q_DECLARE_TR_FUNCTIONS(Bohdan::Main)
};
...
// Helper to filter out AppModule settings that are not useful for BohdanConv application
...
// Initializes and runs Bohdan application
...
{
    qInstallMessageHandler(&Bohdan::LogMessageHandler::qtHandler);
    ...
    QCoreApplication::setApplicationName("BohdanConv");
    QCoreApplication::setApplicationVersion(QString::fromUtf8(Bohdan::strVersion));
    QCoreApplication app(argc, argv);
    return Bohdan::runApp(&app);
}
