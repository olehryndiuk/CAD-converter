...
struct ApplicationI18N {
    MAYO_DECLARE_TEXT_ID_FUNCTIONS(Bohdan::Application)
};
...
void Application::defineBohdanFormat(const ApplicationPtr& app)
{
    const char strFougueCopyright[] = "Copyright (c) 2024, Fougue Ltd. <https://www.fougue.pro>";
    app->DefineFormat(
        Document::NameFormatBinary, ApplicationI18N::textIdTr("Binary Bohdan Document Format").data(), "myb",
        new Document::FormatBinaryRetrievalDriver(app),
        new BinXCAFDrivers_DocumentStorageDriver
    );
    app->DefineFormat(
        Document::NameFormatXml, ApplicationI18N::textIdTr("XML Bohdan Document Format").data(), "myx",
